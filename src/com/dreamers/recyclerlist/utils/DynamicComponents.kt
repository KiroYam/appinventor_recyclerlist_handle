...
class DynamicComponents {
    ...
    // Новый метод для поиска компонента по тегу
    fun getComponentByTag(tag: String): AndroidViewComponent? {
        return androidViews.entries.find { it.key.tag == tag }?.value
    }

    fun getAndroidViewById(id: String): AndroidViewComponent? {
        return uniqueIds.entries.find { it.value == id }?.key
    }

    fun allTags(): List<String> {
        val tagsList = mutableListOf<String>()
        for (view in androidViews.keys) {
            val tag = view.tag?.toString() ?: "null"  // Если тег равен null, добавляем "null"
            tagsList.add(tag)
        }
        return tagsList
    }
            ...
            if (isNotEmptyOrNull(template) && mScheme.has("components")) {
                ...
                for (i in 0 until propertiesArray.length()) {
                    ...
                    val mRoot = if (!mJson.has("in")) `in` else {
                        val viewWithTag = (`in`.view as ViewGroup).findViewWithTag<View>(mJson.getString("in"))
                        androidViews[viewWithTag]!!
                    }
